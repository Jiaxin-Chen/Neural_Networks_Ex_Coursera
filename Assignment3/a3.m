...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  exp_class_input = exp(class_input);
  class_output = exp_class_input ./ repmat(sum(exp_class_input, 1), [size(exp_class_input, 1), 1]);
  
  hid_to_class_err = (class_output - data.targets) / size(data.inputs, 2);
  ret.hid_to_class = hid_to_class_err * hid_output' + model.hid_to_class * wd_coefficient;
  
  input_to_hid_err = (model.hid_to_class' * hid_to_class_err) .* hid_output .* (1 - hid_output);
  ret.input_to_hid = input_to_hid_err * data.inputs' + model.input_to_hid * wd_coefficient;
end
